/* 
*  Copyright (c) 2019. All right reserved
 * Created on 2024-06-19 ( Date ISO 2024-06-19 - Time 16:54:37 )
 * Generated by Telosys Tools Generator ( version 3.3.0 )
 */

/*
 * Created on 2024-06-19 ( 16:54:37 )
 * Generated by Telosys ( http://www.telosys.org/ ) version 3.3.0
 */


package com.maan.eway.bean;


import org.hibernate.annotations.DynamicInsert;
import org.hibernate.annotations.DynamicUpdate;
import jakarta.persistence.*;

import lombok.*;
import jakarta.persistence.*;
import java.io.Serializable;
import java.util.List;

import java.util.Date;
import jakarta.persistence.*;




/**
* Domain class for entity "TermsAndCondition"
*
* @author Telosys Tools Generator
*
*/
@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
@ToString
@Entity
@DynamicInsert
@DynamicUpdate
@Builder
@IdClass(TermsAndConditionId.class)
@Table(name="terms_and_condition")


public class TermsAndCondition implements Serializable {
 
private static final long serialVersionUID = 1L;
 
    //--- ENTITY PRIMARY KEY 
    @Id
    @Column(name="SNO", nullable=false)
    private Integer    sno ;

    @Id
    @Column(name="QUOTE_NO", nullable=false, length=100)
    private String     quoteNo ;

    @Id
    @Column(name="REQUEST_REFERENCE_NO", nullable=false, length=100)
    private String     requestReferenceNo ;

    @Id
    @Column(name="ID", nullable=false)
    private Integer    id ;

    @Id
    @Column(name="SUB_ID", nullable=false)
    private Integer    subId ;

    @Id
    @Column(name="COMPANY_ID", nullable=false, length=20)
    private String     companyId ;

    @Id
    @Column(name="BRANCH_CODE", nullable=false, length=20)
    private String     branchCode ;

    @Id
    @Column(name="PRODUCT_ID", nullable=false, length=20)
    private String     productId ;

    @Id
    @Column(name="SECTION_ID", nullable=false, length=20)
    private String     sectionId ;

    @Id
    @Column(name="RISK_ID", nullable=false, length=20)
    private String     riskId ;

    //--- ENTITY DATA FIELDS 
    @Column(name="ID_DESC", nullable=false, length=100)
    private String     idDesc ;

    @Column(name="SUB_ID_DESC", length=800)
    private String     subIdDesc ;

    @Column(name="COMPANY_NAME", length=100)
    private String     companyName ;

    @Column(name="BRANCH_NAME", length=100)
    private String     branchName ;

    @Column(name="PRODUCT_NAME", length=100)
    private String     productName ;

    @Column(name="SECTION_NAME", length=100)
    private String     sectionName ;

    @Temporal(TemporalType.DATE)
    @Column(name="ENTRY_DATE")
    private Date       entryDate ;

    @Column(name="REMARKS", length=100)
    private String     remarks ;

    @Column(name="STATUS", length=1)
    private String     status ;

    @Column(name="CREATED_BY", length=100)
    private String     createdBy ;

    @Column(name="UPDATED_BY", length=100)
    private String     updatedBy ;

    @Temporal(TemporalType.TIMESTAMP)
    @Column(name="UPDATED_DATE")
    private Date       updatedDate ;

    @Column(name="AMEND_ID")
    private Integer    amendId ;

    @Column(name="DOC_REF_NO", length=100)
    private String     docRefNo ;

    @Column(name="TYPE_ID", length=1)
    private String     typeId ;

    @Column(name="ID_DESC_LOCAL", length=100)
    private String     idDescLocal ;

    @Column(name="SUB_ID_DESC_LOCAL", length=100)
    private String     subIdDescLocal ;

    @Column(name="PRODUCT_NAME_LOCAL", length=100)
    private String     productNameLocal ;

    @Column(name="SECTION_NAME_LOCAL", length=100)
    private String     sectionNameLocal ;

    @Column(name="BRANCH_NAME_LOCAL", length=100)
    private String     branchNameLocal ;


    //--- ENTITY LINKS ( RELATIONSHIP )


}



