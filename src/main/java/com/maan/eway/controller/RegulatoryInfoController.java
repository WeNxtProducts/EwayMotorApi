...
public class RegulatoryInfoController {
	...
	public ResponseEntity<CommonRes> getVehicleInfo(@RequestBody MotorVehicleInfoGetReq req,@RequestHeader("Authorization") String tokens)
	{
		CommonRes data = new CommonRes();
		MotorVehicleInfoRes res = new MotorVehicleInfoRes();
		
		if(StringUtils.isNotBlank(req.getInsuranceId()) && req.getInsuranceId().equalsIgnoreCase("100004")  )
			res = entityService.getVehicleInfoExternal(req,tokens);
		else 
			res = entityService.getVehicleInfo(req,tokens);
		...
		if (res != null) {
			...
			// Find Error Table
			List<Error> error =  entityService.getSearchErrorDetails(req);
...
//			error.add(new Error("01","Not Found", "No Data Found"));
	...
	@PreAuthorize("hasAnyRole('ROLE_APPROVER','ROLE_USER')")
    @PostMapping("/showpolicyholder")
	@ApiOperation("This Method is to get by id")
	public ResponseEntity<CommonRes> showPolicyHolder(@RequestBody MotorVehicleInfoGetReq req,@RequestHeader("Authorization") String tokens)
	{
	CommonRes data = new CommonRes();
	PolicyHolderInfoDto res = entityService.showPolicyHolder(req,tokens);
 ...
 // Get All
	...
	@PreAuthorize("hasAnyRole('ROLE_APPROVER','ROLE_USER')")
    @PostMapping("/savevehicleinfolist")
 	@ApiOperation(value = "This method is Save Regulatory Vehicle Info ")

 	public ResponseEntity<CommonRes> saveVehicleInfoList(@RequestBody MotorVehicleInfoListSaveReq req,@RequestHeader("Authorization") String tokens) {
 		reqPrinter.reqPrint(req);
 		CommonRes data = new CommonRes();
		List<Error> validation = entityService.validateVehicleInfoList(req);
		//// validation
		if (validation != null && validation.size() != 0) {
			data.setCommonResponse(null);
			data.setIsError(true);
			...
			/////// save
			//Save
			MotorSaveRes res = entityService.saveVehicleInfoList(req,tokens);
 	 		data.setCommonResponse(res);
 	 		data.setIsError(false);
 	 		data.setErrorMessage(Collections.emptyList());
 	 		data.setMessage("Success");
	...
	@PreAuthorize("hasAnyRole('ROLE_APPROVER','ROLE_USER')")
	@PostMapping(value="/savevehicleinfocall",produces = "application/json")
 	@ApiOperation(value = "This method is Save Regulatory Vehicle Info ")

 	public ResponseEntity<MotorVehicleInfoCommonRes> saveVehicleInfoListCall(@RequestBody MotorVehicleInfoListSaveReq req,@RequestHeader("Authorization") String tokens) {
 		reqPrinter.reqPrint(req);
 		MotorVehicleInfoCommonRes data = new MotorVehicleInfoCommonRes();
		List<Error> validation = entityService.validateVehicleInfoList(req);
		//// validation
		if (validation != null && validation.size() != 0) {
			data.setCommonResponse(null);
			...
			data.setErrorMessage(validation);
			data.setMessage("Failed");
			return new ResponseEntity<MotorVehicleInfoCommonRes>(data, HttpStatus.OK);

		} else {
			/////// save
			//Save
			MotorSaveRes res = entityService.saveVehicleInfoListCall(req,tokens);
 	 		data.setCommonResponse(res);
 	 		data.setIsError(false);
 	 		data.setErrorMessage(Collections.emptyList());
 	 		data.setMessage("Success");
			if (res != null) {
				return new ResponseEntity<MotorVehicleInfoCommonRes>(data, HttpStatus.CREATED);
			} else {
				return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);
			}
		}
